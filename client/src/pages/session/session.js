...
class Session extends Component {
    ...
    handleSummary = (id) => {
        ...
        for(var tmp in this.state.userdata.courses){
            ...
            console.log("Query: ", this.state.userdata.courses[tmp].course_name, this.props.sessionVars.course)        

            if(this.state.userdata.courses[tmp].course_name ===  this.props.sessionVars.course){
                var query = this.state.userdata.courses[tmp]["keywords"][id]
                console.log("Query 1: ", this.state.userdata.courses[tmp], id)
                console.log("Query 2: ", this.state.userdata.courses[tmp]["keywords"][id])
                const summary = (
    ...
    deletenode = (e) => {
        ...
        // console.log("before",this.state.currentGraph["nodes"])/
        // var node = this.state.currentGraph["nodes"].pop([i])
        // var newNodes = this.state.currentGraph["nodes"]
    ...
    render() { 
        ...
        if(this.state.sessionState !== {}){
            var renderNodes = this.state.sessionState.nodesList.map(
                (topic) => {
                    ...
                    if(this.state.checkList.includes(topic)){
                        ...
                        <li class="list-group-item disabled" id={topic}>
                            {topic} 
                        <button className="btn btn-success btn-sm disabled"  id={topic} onClick={this.deletenode}>completed</button>
                        </li>
                        )
                    }
                    return (
                    <li class="list-group-item" id={topic}>
                        {topic} 
                        <button className="btn btn-success btn-sm active"  id={topic} onClick={this.deletenode}>completed</button>
                    </li>
                    )
                }
            )
            ...
            <div className="container-fluid">
                    <div className="row">
                        <div className="col-4" style={{'marginTop':'1vh'}}>                            
                            ...
                            <h1 className="list-group-item list-group-item-primary" style={{'marginTop':'2vh'}}>
                                Add topics
                            </h1>
                            <button 
                                id = "individual"
